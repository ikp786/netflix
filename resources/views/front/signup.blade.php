        ...
        <div class="lgn-frm sgn-frm "  id="signup_form_div">
            <h3>Sign Up</h3>
              <form class="signup-form" id="signup_form"> 
                ...
                <input type="hidden" name="country_code" id="country_code" value="{{ $sel_country_code}}" />
                <input type="hidden" name="country_flag_code" id="country_flag_code" value="{{ $sel_country_flag_code}}" />
                <!-- <input type="hidden" name="country" id="country" value="" /> -->
                <input type="hidden" name="device_token" id="device_token" value="{{ rand() }}" />

                <input type="text" name="user_name" placeholder="Name">
                <input type="email" name="email" placeholder="Email" required > 
                <input type="text" maxlength="15" value="{{ $sel_mobile}}" name="phone" id="mobile-number" class="mobile-no-coun decimal_number"
                placeholder="" onblur="myFunctionUserLog()" onkeyup="myFunctionUserLog()"  /> 
                ...
                <div class="accept-policy"> <input type="checkbox" name="is_term_accept" id="html" checked /> <p>I agree with <a class="trms-com" href="{{ url('fr_page/1') }}" target="_blank" >Terms & Conditions</a>, <a target="_blank" class="trms-com" href="{{ url('fr_page/2') }}">Policy</a></p></div>
...
@section('script') 
    <script> 
        ...
        function myFunctionUserLog() { 
            var title =   $('#signup_form').find('.selected-flag').attr("title");

            var strArray = title.split("+");      
            var countr_name = strArray[0].trim();
            var countr_code = strArray[1].trim();
            // $('#country').val(countr_name); 
            $('#country_code').val(countr_code); 

            var title_flag_code =   $('#signup_form').find('.selected-flag').children("div").attr("class");
        ...
        $(document).ready(function() {
            ...
            setTimeout(function(){
              country_flag_code = $('#country_flag_code').val(); 
              $('.selected-flag .iti-flag').addClass(country_flag_code);
            }, 1000);
