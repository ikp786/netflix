    ...
    <div class="container text-center" >
        <div class="lgn-frm sgn-frm profile_edit"  id="signup_form_div">
            <h3>Edit Profile</h3>
              ...
              <form id="validate" action="{{ url('save_profile') }}"  class="form-horizontal comm_form " method="POST" role="form" enctype="multipart/form-data" >
                @csrf   
                @if (\Session::has('success'))
                    <div id="message_box">{!! \Session::get('success') !!}</div> 
                @endif

                <input type="hidden" name="country_code" id="userCountryCode" value="{{ old('country_code') ? old('country_code') : $user->country_code }}" />

                <input type="hidden" name="country_flag_code" id="userCountryFlagCode" value="{{ old('country_flag_code') ? old('country_flag_code') : $user->country_flag_code }}" />
                ...
                <input type="number" name="phone" id="mobile-number" value="{{ old('phone') ? old('phone') : $user->phone }}" class="mobile-no-coun decimal_number" onfocus="myFunctionUserLog()" placeholder="Phone" required /> 
                ...
                <input type="file" class="form-control upload-img-file" name="profile_photo_path" >
...
@section('script') 
    ...
    <script> 
      ...
      function myFunctionUserLog() {
        ...
        var flagCodeArr = strArray[0].trim().split(":");
        $('#userCountryFlagCode').val(flagCodeArr[0].trim()); 


          var title =   $('#signup_form_div').find('.selected-flag').attr("title");                
  ...
  var phoneuser = document.querySelector("#mobile-number");
   
  $("#mobile-number").intlTelInput(phoneuser, {
