...
{  
    ...
    { 
        ...
        return Datatables::of($get_data)
            ...
            ->editColumn("video_url",function($get_data){
                return url('/fr_video_detail').'/'.$get_data->id;
            })
    ...
    public function store(Request $request)
    {
        ...
        /*$sub_pprod_arr['sub_category_id'] = $request->sub_category_id;
        $sub_pprod_arr['sub_product_title'] = $request->sub_product_title;
        $sub_pprod_arr['year'] = $request->year;
        $sub_pprod_arr['description'] = $request->description;
        $sub_pprod_arr['status'] = $request->status;*/

        $prop_data = SubProduct::create($input);
        $video_id = $prop_data->id;
        
        if($video_id){

            if($request->language_type_id){
                $lang_input = [];
                foreach($request->language_type_id as $key => $lang_val_id) { 

                    $lang_input['video_id'] = $video_id;
                    $lang_input['language_type_id'] = $lang_val_id; 

                    if(!empty($request->vid_media_url[$key])){
                        $media_path = $request->vid_media_url[$key]->store('media');   
                        $lang_input['media_url'] = $media_path; 
                    }  
                    MoreVideo::create($lang_input);  
                }
            }
        
            if($request->cast_title){
                $video_cast_input = [];
                foreach($request->cast_title as $key => $cast_val) { 

                    $video_cast_input['video_id'] = $video_id;
                    $video_cast_input['cast_title'] = $cast_val; 
                    $video_cast_input['description'] = $request->cast_description[$key]; 

                    if(!empty($request->cast_image[$key])){
                        $cast_media_path = $request->cast_image[$key]->store('media');   
                        $video_cast_input['cast_image'] = $cast_media_path; 
                    } 
                    VideoCasting::create($video_cast_input);  
                }
            }
    ...
    {    
        ...
        $get_more_video_detail = MoreVideo::where('video_id',$get_data->id)->get();
        $get_video_casting_detail = VideoCasting::where('video_id',$get_data->id)->get();

        $get_lang_types = LanguageType::orderBy('language_title','asc')->get(['id','language_title']);

        return view('admin.sub_product.show',compact('get_data','get_product_detail','get_more_video_detail','get_video_casting_detail','get_lang_types'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('admin.sub_product.edit',compact('get_data','GetProduct','get_category','get_more_video_detail','get_video_casting_detail','get_lang_types'));
    }
    ...
    { 
        ...
        $res_data = SubProduct::find($id)->update($input);
            ...
            if($request->language_type_id){
                $lang_input = [];
                // MoreVideo::where(['video_id'=>$id])->delete(); 
                foreach($request->language_type_id as $key => $lang_val_id) { 
          
                    $lang_input['video_id'] = $id;
                    $lang_input['language_type_id'] = $lang_val_id; 
                    ...
                    if(isset($request->more_video_edit_id[$key])){
                        $more_video_edit_id = $request->more_video_edit_id[$key];
                        MoreVideo::where('id',$more_video_edit_id)->update($lang_input);
                    }else{
                        MoreVideo::create($lang_input);  
                    } 
                }
            ...
            if($request->cast_title){
                $video_cast_input = [];
                // VideoCasting::where(['video_id'=>$id])->delete(); 
                foreach($request->cast_title as $key => $cast_val) { 

                    $video_cast_input['video_id'] = $id;
                    $video_cast_input['cast_title'] = $cast_val; 
                    $video_cast_input['description'] = @$request->cast_description[$key]; 
                    ...
                    if(isset($request->cast_video_edit_id[$key])){
                        $cast_video_edit_id = $request->cast_video_edit_id[$key];
                        VideoCasting::find($cast_video_edit_id)->update($video_cast_input);
                    }else{
                        VideoCasting::create($video_cast_input);  
                    }
