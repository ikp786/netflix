...
use App\Http\Resources\MoreVideoCollection;
use App\Models\Page;
...
{ 
    ...
    {    
        ...
        return view('home',compact('get_slider_data','get_trending_data','get_popular_data','get_upcomming_data','get_category_data')); 
    }
    ...
    public function fr_signup(Request $request,$country_selection_data="")
    {    
        $sel_country_code="";$sel_mobile="";
        
        if($country_selection_data!=""){
            $crr = explode('-',$country_selection_data);
            $sel_country_code = $crr[0];
            $sel_country_flag_code = $crr[1];
            $sel_mobile = $crr[2];
        }
        
        $get_country = Country::orderBy('countries_name','asc')->get(['countries_id','countries_name']);
        return view('front.signup',compact('get_country','sel_country_code','sel_country_flag_code','sel_mobile'));
    }
    ...
    {       
        ...
        $sm_record_count = count($get_trending_data)+count($get_popular_data)+count($get_upcomming_data);
        
        $filter_by = (@$request->filter_by) ? @$request->filter_by : ""; 
        return view('video_page',compact('filter_by','prev_sub_category_ids','selected_sub_category','get_sub_category','get_trending_data','get_popular_data','get_upcomming_data','record_type','main_category_ids','sm_record_count')); 
    }
    ...
    public function video_cast_delete($id)
    {  
        VideoCasting::where('id',$id)->delete();
        
        return response()->json(['status'=>1,'message'=>'Video Cast deleted successfully.']);

    } 

    public function more_video_delete($id)
    {  
        MoreVideo::where('id',$id)->delete();
        
        return response()->json(['status'=>1,'message'=>'Video deleted successfully.']);

    }

    public function fr_video_play($video_id,$lang_id="")
    {   
        $bearer_token = Session()->get('bearer_token');
        ...
        if(!empty($lang_id)){ 
            $language_id = $lang_id; 
            $get_data_more_vid = MoreVideo::where(['video_id'=>$video_id,'language_type_id'=>$language_id])->get(); 
        }else
            $get_data_more_vid = MoreVideo::where(['video_id'=>$video_id])->take(1)->get(); 

        $get_data = MoreVideoCollection::collection($get_data_more_vid); 

        $data = json_encode($get_data);
        $video_data = json_decode($data);
 
        $get_video_language = MoreVideo::
            leftjoin('video_language_tbl','video_language_tbl.id','=','more_video_tbl.language_type_id')->
            where('video_id',$video_id)->get(['video_language_tbl.id','video_language_tbl.language_title']);

        $get_video_casting = VideoCasting::orderBy('cast_title','asc')->where(['video_id'=>$video_id])->get(); 

        return view('video_play',compact('video_data','get_video_language','lang_id','get_video_casting')); 
    }

    public function fr_player_test($video_id,$lang_id="")
    {   
        $bearer_token = Session()->get('bearer_token');
        ...
        return view('fr_player_test',compact('video_data','get_video_language','lang_id','get_video_casting')); 
    }

    public function fr_more_video_api(Request $request){
        ...
        {   
            ...
            if(isset($request->language_type_id)){
                $language_id = $request->language_type_id; 
                $get_data_more_vid = MoreVideo::where(['video_id'=>$video_id,'language_type_id'=>$language_id])->get(); 
            }else
                $get_data_more_vid = MoreVideo::where(['video_id'=>$video_id])->take(1)->get(); 

            $res_data = MoreVideoCollection::collection($get_data_more_vid); 
