...
{   
    public function get_sub_category(Request $request){
        ...
        {   
            if($request->category_id='all'){
                $getDt =Product::orderBy('id','desc')->where(['status'=>1])->get();
            }else{
                $getDt =Product::orderBy('id','desc')->where(['category_id'=>$request->category_id,'status'=>1])->get();
            }
            

            $res_data = ProductApiCollection::collection($getDt); 
    ...
    public function get_user_profile(Request $request) {
        ...
        $get_slide = Slider::orderBy('id','desc')->where(['status'=>'1','slider_type'=>1])->first(['slider_image']); 
        ...
        $req_data = [
            ...
            'banner_image' => (@$get_slide->slider_image) ? url('uploads/'.@$get_slide->slider_image) : "",             
    ...
    public function get_language_type(){ 
        $res_data = LanguageType::orderBy('language_title','asc')->get()->toArray(); 
        $req_data = $res_data;  
        $req_message = "Record Found"; 
    ...
    public function get_home_list(Request $request) {
        ...
        { 
            ...
            $get_slide = Slider::orderBy('id','desc')->where('status','1')->get(['id','slider_image','video_id','video_url','status']); 
            ...
            if($request->category_id=="all"){
                ...
                $get_trending_data =  SubProduct::orderBy('id','desc')->whereIn('product_id',$getSubbCatIds)->where(['status'=>'1','video_type'=>1])->get();
                $get_popular_data =  SubProduct::orderBy('id','desc')->whereIn('product_id',$getSubbCatIds)->where(['status'=>'1','video_type'=>2])->get(); 
                $get_upcomming_data =  SubProduct::orderBy('id','desc')->whereIn('product_id',$getSubbCatIds)->where(['status'=>'1','video_type'=>3])->get();
            }    
    ...
    public function add_to_my_list(Request $request) {
        ...
        {    
            ...
            if($is_fav==0){ 
                ...
                $req_message = "Video Added To My Favourite List";
                $req_data['is_added']='1';
            }else{
                Favourite::where(['user_id' => $user_id,'sub_product_id' => $request->video_id])->delete();
                $req_message = "Video Removed From My Favourite List"; 
            }    
        }
    ...
    public function get_my_video_list(Request $request) {
        ...
        $trenQue =  SubProduct::orderBy('id','desc')->where(['sub_products.video_type'=>1,'status'=>'1']); 
        $popQue =  SubProduct::orderBy('id','desc')->where(['sub_products.video_type'=>2,'status'=>'1']);
        $uppQue =  SubProduct::orderBy('id','desc')->where(['sub_products.video_type'=>3,'status'=>'1']);
