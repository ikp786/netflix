...
{ 
    ...
    {        
        ...
        $prev_sub_category_ids = $request->prev_sub_category_ids ?? '';

        $QueMnCat = Product::orderBy('product_name','asc')->where(['status'=>'1']);

        $Que1 = Product::orderBy('product_name','asc')->where(['status'=>'1']);
        ...
        if($request->filter_by){
            ...
            $prev_sub_category_ids = $request->prev_sub_category_ids;
            if($request->filter_by=="category_search" || $request->filter_by=="search_by_all"){  
               
                $idss = explode(',',$prev_sub_category_ids);   
                if(count($idss)>0){   
                    $Que1->whereIn('id',$idss);  
                    $QueMnCat->whereIn('id',$idss);
                }else{ 
                    $Que1->where('id',$prev_sub_category_ids); 
                    $QueMnCat->where('id',$prev_sub_category_ids);
                }  
            }
        } 
     
        $getMainCat = $QueMnCat->groupBy('category_id')->pluck('category_id')->toArray();
        $main_category_ids = implode(",", $getMainCat);
        
        $QueSubCrr = Product::where(['status'=>'1']);
        if($request->search_by_sub_cat_id){ 
            $subb_catt = $request->search_by_sub_cat_id;
            $sub_idss = explode(',',$subb_catt);   
            if(count($sub_idss)>0){ 
                $QueSubCrr->whereIn('id',$sub_idss);   
            }else{  
                $QueSubCrr->where('id',$subb_catt);  
            }  
        }else if($request->filter_by=="search_by_all"){ 
            $prev_idss = explode(',',$prev_sub_category_ids);   
            if(count($prev_idss)>0){   
                $QueSubCrr->whereIn('id',$prev_idss);   
            }else{ 
                $QueSubCrr->where('id',$prev_sub_category_ids);  
            }
        }
        $getSubbCat = $QueSubCrr->pluck('id')->toArray();
        
        $get_category = $Que1->get();  
        $sub_catt_arr = $get_category->pluck('id')->toArray();
        
        if(count($getSubbCat)>0){ 
            $Que2->whereIn('product_id',$getSubbCat);
            $Que3->whereIn('product_id',$getSubbCat);
            $Que4->whereIn('product_id',$getSubbCat);
        }
        ...
        $selected_sub_category = $request->search_by_sub_cat_id;
        $record_type = ($request->filter_by=='search_by_all') ? "all" : '';

        return view('video_page',compact('prev_sub_category_ids','selected_sub_category','get_category','get_trending_data','get_popular_data','get_upcomming_data','record_type','main_category_ids')); 
    }
